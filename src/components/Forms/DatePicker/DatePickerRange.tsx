...
const DatePickerRange = ({ value, onChange }: DatePickerRangeProps) => {
  ...
  useEffect(() => {
    ...
    fpInstance.current = flatpickr(inputRef.current, {
      ...
      defaultDate: value && value[0] && value[1] ? value : undefined,
      onChange: (selectedDates) => {
        // Flatpickrの内部UI更新（日付のハイライトなど）がReactの再描画によって中断されないよう、
        // State更新をイベントループの次に遅延させます。
        setTimeout(() => {
          ...
          if (selectedDates.length === 2) {
            onChange?.([formatDate(selectedDates[0]), formatDate(selectedDates[1])]);
  ...
  // value の変化を監視して UI 更新 (外部からのリセット対応)
  useEffect(() => {
    if (fpInstance.current) {
      if (!value || (!value[0] && !value[1])) {
        // Only clear if both are empty AND flatpickr is not already empty
        if (fpInstance.current.selectedDates.length > 0) {
          fpInstance.current.clear();
        }
      } else if (value[0] && value[1]) {
        // If external state is a full range, and internal flatpickr differ, set it
        // (This prevents breaking the UI when the user is mid-selection)
